...
const onClickPagination = (e) => {
    ...
    switch (page) {
        case "Trang đầu":
            getProductList({ page: 1, limit, ...(sort && { sort }), ...(typeProduct && { typeProduct }), ...(rangePrice && { rangePrice }) });
            break;
        case "Trang cuối":
            page = paginationArr.length - 2;
            getProductList({ page, limit, ...(sort && { sort }), ...(typeProduct && { typeProduct }), ...(rangePrice && { rangePrice }) });
            break;
        default:
            getProductList({ page, limit, ...(sort && { sort }), ...(typeProduct && { typeProduct }), ...(rangePrice && { rangePrice }) });
    }
...
$(".portfolio-product__item").click((e) => {
    typeProduct = e.target.parentElement.value;
    console.log(e.target);
    getProductList({ page: 1, limit, typeProduct, ...(sort && { sort }) });
...
$(".portfolio-price__item").click((e) => {
    ...
    getProductList({ page: 1, limit, rangePrice, ...(typeProduct && { typeProduct }), ...(sort && { sort }) });
...
const nextPage = (id) => {
    window.location.href = `http://localhost:3001/productDetail.html?${id}`;
};
